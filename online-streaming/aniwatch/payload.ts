...
class Provider {
    ...
    readonly ANIWATCH_SEARCH_URL = "https://aniwatchtv.to/ajax/search/suggest?keyword="
    readonly ANIWATCH_MAIN_URL = "https://aniwatchtv.to";
    readonly ANIWATCH_EPISODE_URL = "https://aniwatchtv.to/ajax/v2/episode/list/"
    readonly ANIWATCH_EPISODE_SERVER_URL = `https://aniwatchtv.to/ajax/v2/episode/servers?episodeId=`;
    readonly ANIWATCH_SERVER_URL = `https://aniwatchtv.to/ajax/v2/episode/sources?id=`
    ...
    async search(opts: SearchOptions): Promise<SearchResult[]> {
        ...
        const searchPagePromises = queryVariants.map(async (variant) => {
            ...
            const html = await fetch(`${this.ANIWATCH_SEARCH_URL}${encodeURIComponent(variant)}`).then(res => res.text()).then(text => JSON.parse(text) as { html: string }).then(json => json.html);

            if (html.includes("<span>No results found!</span>") || html.length === 0) {
                console.log("No results found for:", variant);
                return undefined;
            }
            else {
                const $ = await LoadDoc(html);
                const movies = $("a.nav-item").filter((i, el) => !el.attr("class")?.includes("nav-bottom"));
                if (movies.length() === 0) return undefined;
                const moviesVariants = movies.map((i, el) => {
                    const url = el.attr("href");
                    const title = el.find("h3.film-name").text();
                    return { url, title };
                });

                const moviesPromises = moviesVariants.map(async ({ url, title }) => {
                    const $2 = await fetch(`${this.ANIWATCH_MAIN_URL}${url}`)
                        .then(req => req.text())
                        .then(LoadDoc);

                    // remove anime that don't have the correct language
                    const HasLangVersion = opts.dub ? $2("div#ani_detail div.tick-dub").length() > 0 : $2("div#ani_detail div.tick-sub").length() > 0;
                    if (!HasLangVersion) {
                        return undefined;
                    }
                    ...
                    if (title && url) {
                        ...
                        if ((seasonNumberOpts && seasonNumberOpts === seasonNum) ||
                            ...
                            console.log("Not the right season:", seasonNumberOpts, seasonNum, partNumberOpts, partNum, onylNum);
                        }
                    }
                    return undefined;
                });

                const moviesResult = (await Promise.all(moviesPromises)).filter(r => r !== undefined) as { Title: string; Url: string }[];

                return { moviesResult, variant };
            }
        });
        ...
        results.forEach((result, index) => {
            results.forEach((otherResult, otherIndex) => {
                if (index !== otherIndex && result && otherResult) {
                    result.moviesResult.forEach((movie) => {
                        otherResult.moviesResult = otherResult.moviesResult.filter((otherMovie) => otherMovie.Url !== movie.Url);
                    });
                }
            });
        });
        ...
        const bestMovie = this.findBestTitle(results.filter(r => r !== undefined).map(r => r!.moviesResult).flat(), opts.query);
        if (bestMovie) {
            return [{
                id: bestMovie.Url + "/" + opts.dub,
                title: bestMovie.Title,
                url: this.ANIWATCH_MAIN_URL + bestMovie.Url,
    ...
    async findEpisodes(id: string): Promise<EpisodeDetails[]> {
        const dub = id.split("/")[1] === "true" ? "dub" : "sub";
        const animeID = id.match(/\d+/);
        console.log(`${this.ANIWATCH_EPISODE_URL}${animeID}`);
        const response = await fetch(`${this.ANIWATCH_EPISODE_URL}${animeID}`, {
            headers: {
        ...
        const $ = LoadDoc(json.html);


        const ServerToAdd: string[] = [];
        const epsUrl = $(".ss-list .ssl-item.ep-item");

        const episodeDetails = await Promise.all(
            epsUrl.map(async (_, el) => {
                const href = el.attr("href")?.split('/watch/')[1];
                const title = el.find('.e-dynamic-name').text().trim();
                const episodeNumber = el.find('.ssli-order').text().trim();
                if (!href) return undefined;
                const episodeId = href.match(/ep=(\d+)/)?.[1];
                let req = await fetch(`${this.ANIWATCH_EPISODE_SERVER_URL}${episodeId}`)
                if (req.status === 503) {
                ...
                const episodeDoc = LoadDoc(json.html);

                const serverItems = episodeDoc(`.ps_-block.ps_-block-sub.servers-${dub} .ps__-list .server-item`)
                let Servers = "";

                serverItems.each((_, serverEl) => {
                    const server = serverEl.find('a').text().toLowerCase().trim();
                    const srcId = serverEl?.attr('data-id')?.trim();

                    if (srcId && server) {
                        Servers += server + ":" + srcId + ",";

                        if (DevMode) {
                            // check if servername is in the list of the episode servers
                            if (server !== undefined && !this.getSettings().episodeServers.includes(server) && !ServerToAdd.includes(server)) {
                                ServerToAdd.push(server);
                            }
                        }
                    }
                });
    ...
    async findEpisodeServer(episode: EpisodeDetails, _server: string): Promise<EpisodeServer> {
        ...
        const CurrentServer = servers.find(s => s.includes(_server));
        const ID = CurrentServer?.split(":")[1];
        const server = CurrentServer?.split(":")[0];
        const serverUrl = await fetch(`${this.ANIWATCH_SERVER_URL}${ID}`)
            .then(req => req.text())
